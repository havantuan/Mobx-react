...
export class NotificationStore {
  ...
  fetch = (filter, pagination, order = []) => {
    let query = `
        query Notifications($Page: Int = 1, $Limit: Int = 10, $Order: [Sort],$Query: String, $ID: Int, $AppType:  EnumAppType) {
  ...
  fetchByID = (id) => {
    ...
    return authRequest
      .post(apiUrl.GRAPH_URL, {
        ...
        this.currentRow = result.data.Notification;
      })).catch(action(e => {
        errorMessage(e);
        throw e;
      })).finally(action(() => {
